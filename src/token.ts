...
export class TokenizedExpression {
    ...
    public toExpressionDefinitions(isOptimistic = false): ParsedExpressionDefinitions {
        ...
        for (const tok of TokenizedExpression.trimWhitespacesAndRemoveComments(this.tokens)) {
            try {
                switch (state) {
                    ...
                    case State.SkipToCommandOrEOF:
                        if (!isOptimistic) throw new Error('GetCommandOrEOF can only be reached in optimistic mode');

                        if (tok.type == TokenType.CmdSpecifier) {
                            state = State.GetCommandName;
                        } else {
                            state = State.GetCommandOrEOF;
                        }
                ...
                if (isOptimistic) {
                    state = State.SkipToCommandOrEOF;
                } else {
                    throw e
                }
            }
